...
const CONFIG_MAP: readonly ConfigMapping[] = [
  ...
  // OpenAI-compatible providers are adapted through a local Anthropic-compatible proxy.
  { env: 'SHANNON_AI_PROVIDER', toml: 'openai_compatible.provider', type: 'string' },
  { env: 'OPENAI_COMPAT_API_KEY', toml: 'openai_compatible.api_key', type: 'string' },
  { env: 'OPENAI_COMPAT_BASE_URL', toml: 'openai_compatible.base_url', type: 'string' },
  { env: 'OPENAI_COMPAT_SMALL_MODEL', toml: 'openai_compatible.small_model', type: 'string' },
  { env: 'OPENAI_COMPAT_MEDIUM_MODEL', toml: 'openai_compatible.medium_model', type: 'string' },
  { env: 'OPENAI_COMPAT_LARGE_MODEL', toml: 'openai_compatible.large_model', type: 'string' },
  { env: 'OPENAI_COMPAT_REASON', toml: 'openai_compatible.reason', type: 'string' },
  { env: 'SHANNON_OPENAI_PROXY_PORT', toml: 'openai_compatible.proxy_port', type: 'number' },
...
function validateProviderFields(config: TOMLConfig, provider: string, errors: string[]): void {
  ...
  switch (provider) {
    ...
    case 'openai_compatible': {
      const required = ['provider', 'api_key'];
      const missing = required.filter((k) => !keys.includes(k));
      if (missing.length > 0) {
        errors.push(`[openai_compatible] missing required keys: ${missing.join(', ')}`);
      }
      break;
    }
...
function validateConfig(config: TOMLConfig): string[] {
  ...
  const PROVIDER_SECTIONS = ['anthropic', 'openai_compatible', 'custom_base_url', 'bedrock', 'vertex'] as const;
