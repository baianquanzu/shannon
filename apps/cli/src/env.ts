...
interface CredentialValidation {
  valid: boolean;
  error?: string;
  mode: 'api-key' | 'oauth' | 'custom-base-url' | 'openai-compatible' | 'bedrock' | 'vertex';
}
...
function detectProviders(): string[] {
  ...
  if (
    process.env.SHANNON_AI_PROVIDER ||
  ...
  if (isCustomBaseUrlConfigured() && !providers.includes('OpenAI-compatible')) providers.push('Custom Base URL');
...
export function validateCredentials(): CredentialValidation {
  ...
  if (isCustomBaseUrlConfigured()) {
    if (
      ...
      return { valid: true, mode: 'openai-compatible' };
    }
