...
type Provider = 'anthropic' | 'openai_compatible' | 'custom_base_url' | 'bedrock' | 'vertex';
...
export async function setup(): Promise<void> {
  ...
  const provider = await p.select({
    ...
    options: [
      ...
      { value: 'openai_compatible' as const, label: 'GPT / DeepSeek', hint: 'OpenAI-compatible adapter' },
...
async function setupOpenAICompatible(): Promise<ShannonConfig> {
  const provider = await p.select({
    message: 'OpenAI-compatible provider',
    options: [
      { value: 'openai' as const, label: 'OpenAI GPT' },
      { value: 'deepseek' as const, label: 'DeepSeek' },
      { value: 'openai-compatible' as const, label: 'Other compatible endpoint' },
    ],
  });
  if (p.isCancel(provider)) return cancelAndExit();

  const apiKey = await promptSecret(
    provider === 'deepseek' ? 'Enter your DeepSeek API key' : 'Enter your OpenAI-compatible API key',
  );
  ...
  const modelDefaults =
    provider === 'deepseek'
      ? { small: 'deepseek-chat', medium: 'deepseek-chat', large: 'deepseek-reasoner' }
      : { small: 'gpt-4o-mini', medium: 'gpt-4o', large: 'gpt-4o' };

  const customizeModels = await p.confirm({
    message:
      'Do you want to change the default OpenAI-compatible models?\n' +
      `    Small  - ${modelDefaults.small}\n` +
      `    Medium - ${modelDefaults.medium}\n` +
      `    Large  - ${modelDefaults.large}`,
    initialValue: false,
  });
  ...
  if (customizeModels) {
    const small = await p.text({
      ...
      initialValue: modelDefaults.small,
    ...
    const medium = await p.text({
      ...
      initialValue: modelDefaults.medium,
    ...
    const large = await p.text({
      ...
      initialValue: modelDefaults.large,
